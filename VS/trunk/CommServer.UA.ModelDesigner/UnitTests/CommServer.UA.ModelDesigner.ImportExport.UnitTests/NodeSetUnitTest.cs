...
namespace CAS.UA.Model.Designer.ImportExport.UT
{
  ...
  public class NodeSetUnitTest
  {
    ...
    public void UADataTypeTestMethod()
    {
      ...
      List<TraceMessage> _trace = new List<TraceMessage>();
      int _diagnosticCounter = 0;
      ModelDesign _md = NodeSet.AddressSpaceContextService.CreateInstance(_testDataFileInfo, z => TraceDiagnostic(z, _trace, ref _diagnosticCounter));
      Assert.AreEqual<int>(0, _trace.Where<TraceMessage>(x => x.BuildError.Focus != Focus.Diagnostic).Count<TraceMessage>());
    ...
    private static void Compare(Reference[] expected, Reference[] actual)
    {
      ...
      Assert.IsNotNull(actual);
      Assert.AreEqual<int>(expected.Length, actual.Length);
      Assert.IsTrue(!expected.Where<Opc.Ua.ModelCompiler.Reference>(x => x.ReferenceType == null || x.ReferenceType.IsEmpty).Any<Opc.Ua.ModelCompiler.Reference>());
      Assert.IsTrue(!actual.Where<Opc.Ua.ModelCompiler.Reference>(x => x.ReferenceType == null || x.ReferenceType.IsEmpty).Any<Opc.Ua.ModelCompiler.Reference>());
      Assert.IsTrue(!expected.Where<Opc.Ua.ModelCompiler.Reference>(x => x.TargetId == null || x.TargetId.IsEmpty).Any<Opc.Ua.ModelCompiler.Reference>());
      Assert.IsTrue(!actual.Where<Opc.Ua.ModelCompiler.Reference>(x => x.TargetId == null || x.TargetId.IsEmpty).Any<Opc.Ua.ModelCompiler.Reference>());
      Dictionary<string, Opc.Ua.ModelCompiler.Reference> _actualDictionary = actual.ToDictionary<Opc.Ua.ModelCompiler.Reference, string>(x => x.Key());
      foreach (Opc.Ua.ModelCompiler.Reference _rf in expected)
      {
