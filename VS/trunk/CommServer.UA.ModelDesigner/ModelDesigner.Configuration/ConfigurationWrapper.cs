//<summary>
//  Title   : ConfigurationWrapper class is to be used as user interface in the <see cref="System.Windows.Forms.PropertyGrid"/>.
...
namespace CAS.CommServer.UA.ModelDesigner.Configuration
{
  ...
  public class ConfigurationWrapper
  {
    ...
    public ConfigurationWrapper( FileInfo file, IConfiguration configurationEditor )
    {
      ...
      {
        MessageBox.Show( Resources.ConfigurationWrapper_NoConfigurationFileSelectedInfo, Resources.ConfigurationWrapper_NoConfigurationFileSelectedHeader, MessageBoxButtons.OK, MessageBoxIcon.Exclamation );
        FolderBrowserDialog fbd = new FolderBrowserDialog();
        fbd.ShowDialog();
        string selectedPath = fbd.SelectedPath;
        if ( string.IsNullOrEmpty( selectedPath ) )
        {
          MessageBox.Show( Resources.ConfigurationWrapperNoFolderSelectedInfo, Resources.ConfigurationWrapperNoFolderSelectedHeader, MessageBoxButtons.OK, MessageBoxIcon.Exclamation );
        ...
        try
        {
          file = new FileInfo( Path.Combine( fbd.SelectedPath, configurationEditor.DefaultFileName ) );
        }
        catch ( Exception exception )
        {
          MessageBox.Show( Resources.ConfigurationWrapperProblemWithOpeningOfTheFileInfo + exception.Message, Resources.ConfigurationWrapperProblemWithOpeningOfTheFileHeader, MessageBoxButtons.OK, MessageBoxIcon.Error );
        }
    ...
    /// </summary>
    /// <param name="configurationEditor">The server configuration editor main interface.</param>
    public ConfigurationWrapper( IConfiguration configurationEditor ) :
      this( null, configurationEditor ) { }
    #endregion
    ...
    private void SetFileDialogDefaults( FileDialog ofg )
    {

      try
      {
        ofg.DefaultExt = Path.GetExtension( m_ServerConfiguration.DefaultFileName );
      ...
      {
        MessageBox.Show( ex.Message, Resources.UAServerConfigurationTitle, MessageBoxButtons.OK, MessageBoxIcon.Error );
    ...
    private FileInfo FindConfigurationFile()
    {
      ...
      {
        SetFileDialogDefaults( ofg );
        //TODO add icon 
        if ( ofg.ShowDialog() == DialogResult.OK && !String.IsNullOrEmpty( ofg.FileName ) )
          newFile = new FileInfo( ofg.FileName );
      }
